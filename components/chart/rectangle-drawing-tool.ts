...
const defaultOptions: RectangleDrawingToolOptions = {
    ...
    showLabels: true,
    priceLabelFormatter: (price: number) => price.toFixed(4),
...
export class RectangleDrawingTool {
    ...
    public setDrawing(enabled: boolean) {
        if (enabled) {
            this.startDrawing();
        } else {
            this.stopDrawing();
    ...
    public setColor(color: string) {
        this._defaultOptions.fillColor = color + 'CC';
        this._defaultOptions.previewFillColor = color + '77';
        this._defaultOptions.labelColor = color;
    }

    // Add a handler for rectangle selection
    public handleRectangleClick(rectangle: Rectangle) {
        this._selectedRectangle = rectangle;
        // Optionally highlight or show edit handles
        this._selectedRectangle.applyOptions({ fillColor: 'rgba(255, 200, 50, 0.75)' }); // Example highlight
    }

    public editSelectedRectangle(newP1: Point, newP2: Point) {
        if (!this._selectedRectangle) return;
        this._selectedRectangle._p1 = newP1;
        this._selectedRectangle._p2 = newP2;
        this._selectedRectangle.updateAllViews();
    ...
    public clearSelection() {
        if (this._selectedRectangle) {
            this._selectedRectangle.applyOptions({ fillColor: this._selectedRectangle._options.fillColor });
        }
    ...
    private _onClick(param: MouseEventParams) {
        console.log("RectangleDrawingTool: Click event", this._rectangles);
